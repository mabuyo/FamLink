...
class LocationsDataSource {
    ...
    func populateData() {
        if let locationsLoading = (NSKeyedUnarchiver.unarchiveObject(withFile: Location.archiveURL.path) as? [Location]) {
            ...
            let home = Location(name: "home", icon: UIImage(named: "home")!, number: 12, placemark: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: 53.485179, longitude: -113.504746)), identifier: NSUUID().uuidString)
        ...
        for location in self.locations {
            print("Location \(location.name) with identifier \(location.identifier)")
        }
    }
